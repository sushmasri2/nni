...
class user_hierarchy
{
    ...
    private static function get_users_with_completion($field_id, $value, $exclude_self = true)
    {
        ...
        return $DB->get_records_sql("
            SELECT DISTINCT u.*,
                   ...
                   CONCAT(no_user.firstname, ' ', no_user.lastname) AS nutrition_officer,
                   CONCAT(rh_user.firstname, ' ', rh_user.lastname) AS regional_head,
            ...
            LEFT JOIN {user} spoc_user ON spoc_user.username = ud8.data
            LEFT JOIN {user} am_user ON am_user.username = ud11.data  
            LEFT JOIN {user} no_user ON no_user.username = ud12.data
            LEFT JOIN {user} rh_user ON rh_user.username = ud13.data
     ...
     */
    public static function get_current_user_role($userid = 0)
    {
        global $USER;
        if ($userid === 0) {
            $userid = $USER->id;
